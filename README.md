...
13. **`Regelung aktiv = Aus`.** Beim Ausschalten erzwingt die Integration Ausgang → 0 W, Entladestrom → Max-Entladestrom und Ruhemodus (Punkt 15, Standard `'0'`) — innerhalb desselben Locks wie der Regelzyklus, damit kein laufender Zyklus den Disabled-Befehl überschreibt. Danach blockt der Guard jeden weiteren Schreibbefehl, auch den Entladestrom-Abgleich aus Punkt 12. Der Sensor „Betriebsmodus“ wird explizit auf `disabled_regulation_off` bzw. `rest_discharge_regulation_off` gesetzt. Die Flags `cycle_active` und `tariff_charge_active` bleiben bewusst erhalten, damit ein Aus/Ein-Wechsel den Regelkontext nicht verliert. Gilt identisch für Switch-Entität und Panel-Button.
...
15. **Ruhezustand.** Alle Übergänge, in denen die Regelung ruht — Fall B, C, F, TM, Lade-Ende (HT, H) und Fall I ohne aktiven Zyklus, Stillstands-Recovery aus Punkt 14 und `Regelung aktiv = Aus` aus Punkt 13 — schreiben Output 0 W und den **Ruhemodus**. Standard ist Modus `'0'`; Modus `'0'` wird ausschließlich auf diesem Weg geschrieben. Mit dem Schalter **Ruhezustand in Modus 1** im Debug-Tab ist der Ruhemodus stattdessen `'1'` mit 0 W, Modus `'0'` wird dann nie geschrieben. Die Ruhe in Modus `'1'` trägt ein eigenes Zustandsflag: in ihr läuft kein PI, die Instanz zählt nicht zum Entlade-Pool der Verteilung, Fall C, F und TM lösen nicht erneut aus, und Fall E, A, D, 0A, G und GT holen sie zurück wie aus Modus `'0'`. Der Betriebsmodus-Sensor zeigt `rest_discharge` bzw. `rest_discharge_regulation_off`. Gedacht für Geräte, die in Modus `'0'` nicht laden. **Nicht empfohlen:** das Gerät verlässt die Fernsteuerung nie mehr, auch nicht bei ausgeschalteter Regelung. Umschalten wirkt im nächsten Zyklus — eine ruhende Instanz wechselt dann einmal in den anderen Ruhemodus.
