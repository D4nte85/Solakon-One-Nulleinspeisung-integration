...
Bei nur einer Gruppe (Normalfall) ist diese Zuordnung unsichtbar — es gibt genau einen gemeinsamen Verteilungs-Tab. Bei mehreren Gruppen ersetzt die oberste Tab-Ebene die einzelnen Instanz-Tabs durch Bereichs-Tabs (`Bereich: <Name>`); jede Gruppe öffnet darunter ihre eigene zweite Tab-Ebene mit „Verteilung" und den zugehörigen Instanzen. Die Übersichtsseite zeigt weiterhin alle Instanzen aller Gruppen, untereinander nach Gruppe sortiert mit einer eigenen **Gesamt-Karte** je Gruppe mit mehr als einer Instanz: SOC-Mittelwert (kapazitätsgewichtet, wenn bei allen Instanzen der Gruppe ein gültiger Kapazitätssensor gesetzt ist, sonst ungewichteter Mittelwert — Kennzeichnung „⌀"), Summe der Ausgangsleistung, Netzleistung (direkt vom gemeinsamen Netzsensor) sowie der aktuelle Verteilungs-Modus. Degradiert der Coordinator den konfigurierten Modus diesen Zyklus mangels gültigem Fremdinstanz-Sensor (siehe unten), zeigt die Karte beide Modi (`Kapazitätsgewichtet → SOC-gewichtet ⚠️`).

**Anzeigename einer Gruppe:** der Home-Assistant-Bereich des Netzsensors — direkt zugewiesen oder über sein Gerät geerbt. Ohne Bereich wird der Anzeigename des Sensors verwendet, ersatzweise seine Entity-ID. Bei der von Home Assistant empfohlenen Entitätsbenennung (Bereich, Gerät, Entität) heißen die Netzsensoren mehrerer Stromkreise sonst alle gleich (z. B. „Netz Leistung") und die Gruppen wären nicht unterscheidbar. Die Gruppenzuordnung selbst bleibt unverändert an der Sensor-Entität — der Bereich ist reine Beschriftung.

**Zustandskennzeichnung der Instanz-Karten:** Der farbige Kartenkopf zeigt die aktive Zone. Zusätzlich wird eine Karte ausgegraut, solange die Regelung dieser Instanz inaktiv ist, und erhält bei anliegender Fehlermeldung einen roten Rahmen samt ⚠️ hinter dem Namen — die Meldung selbst erscheint als Tooltip auf der Karte und im Status-Tab.
...
Die App wird nur für diese Einstellung gebraucht, danach läuft das Gerät ohne sie. Lädt der Akku in Zone 3 bei PV-Leistung nicht, fehlt meist eine dieser Einstellungen.
...
| PV-Ladereserve (W) | Zone-2-Output-Limit: `min(Hard-Limit Z1, max(0, PV − Reserve))`. Dient auch als Ausschaltschwelle der Nachtabschaltung | 30–100 |
...
Drei Preisstufen: **Günstig** (Preis < Günstig-Schwelle): Tarif-Laden mit fester Leistung bis SOC-Ziel — liegt der SOC am Ladeziel oder weniger als die SOC-Hysterese darunter, greift stattdessen der Discharge-Lock. **Mittel** (Günstig ≤ Preis < Teuer): Discharge-Lock — Zone 1 und Zone 2 gesperrt (Output 0 W, Modus Disabled). Der Discharge-Lock gilt für **beide** Stufen (günstig + mittel), also alles unterhalb der Teuer-Schwelle. Wenn der Preis die Teuer-Schwelle überschreitet, wird der Betrieb automatisch wiederhergestellt. **Teuer** (Preis ≥ Teuer-Schwelle): normale SOC-Logik, keine Einschränkung.
...
**Mehrere Instanzen am selben Netzsensor:** Der StdDev-Ringpuffer gehört der Netzgruppe. Nur der Gruppen-Leader (kleinste entry_id unter den aktiv regelnden Instanzen) fügt Messwerte hinzu, alle anderen übernehmen seinen Wert. Wechselt der Leader, läuft der Puffer ohne Neustart weiter. Verhindert, dass mehrere unabhängig berechnete, leicht phasenversetzte StdDev-Historien sich gegenseitig hochschaukeln (siehe Troubleshooting). Bei Einzelbetrieb ist die Instanz immer ihr eigener Leader — kein Unterschied zum bisherigen Verhalten.
...
| **H** — AC Laden Ende | Modus = `'3'` UND `ac_charge_active = True` UND kein Tarif-Laden UND (**AC-Option AUS** ODER SOC ≥ Ladeziel ODER (Grid ≥ ac_offset + Hysterese UND \|eigener Output\| ≤ Toleranz)) | `ac_charge_active → False`. Integral = 0. Zone 1 → Timer-Toggle + `'1'` / Zone 2 → Timer-Toggle + `'0'` + 0 W. |
...
12. **Entladestrom-Abgleich.** Der maximale Entladestrom wird in jedem Zyklus zentral aus dem Regelzustand abgeleitet (Surplus → 2 A, AC-/Tarif-Laden → 0 A, Zone 1 → Max-Entladestrom, Zone 2 und Ruhe in Modus `'1'` → 0 A, sonst — Ruhe in Modus `'0'`, Tarifsperre — Max-Entladestrom, damit das Gerät beim Verlassen der Fernsteuerung nicht mit 0 A stehen bleibt) und mit dem Ist-Wert abgeglichen — vor dem PI-Gate, also auch im Disabled-Leerlauf. Das garantiert, dass ein Klemmwert (insb. die 2 A aus dem Surplus-Trick) nie über einen Zustandswechsel hinaus stehen bleibt und die Batterie drosselt; die einzelnen Falls setzen den Strom nicht mehr selbst.
13. **`Regelung aktiv = Aus`.** Beim Ausschalten erzwingt die Integration Ausgang → 0 W, Entladestrom → Max-Entladestrom und Modus `'0'` — unabhängig vom Schalter **Ruhezustand in Modus 1** (Punkt 15), damit das Gerät die Fernsteuerung verlässt — innerhalb desselben Locks wie der Regelzyklus, damit kein laufender Zyklus den Disabled-Befehl überschreibt. Danach blockt der Guard jeden weiteren Schreibbefehl, auch den Entladestrom-Abgleich aus Punkt 12. Der Sensor „Betriebsmodus“ wird explizit auf `disabled_regulation_off` gesetzt. Die Flags `cycle_active` und `tariff_charge_active` bleiben bewusst erhalten, damit ein Aus/Ein-Wechsel den Regelkontext nicht verliert. Nach dem Wiedereinschalten fängt die Falltabelle jede Lage, die daraus entstehen kann: eine Lade-Session, deren Option inzwischen aus ist, beenden Fall HT bzw. H; einen Widerspruch zwischen Modus und Lade-Flags räumt Fall I. Gilt identisch für Switch-Entität und Panel-Button.
14. **Ausgangs-Stillstandserkennung.** Steht der Sollwert am oberen Limit (`dynamic_max`) und besteht der Netzfehler in dieselbe Richtung fort, kann der PI nicht weiter hochregeln und schreibt nicht mehr. Bleibt der Wechselrichter in genau diesem Zustand stehen, ohne den Modus zu wechseln, erhält er von sich aus keinen neuen Befehl mehr — Fall D greift nicht, da er einen Modus außerhalb `'1'`/`'3'` voraussetzt. Die Regelung prüft deshalb in diesem gesättigten Zustand die **tatsächliche Ausgangsleistung gegen das Limit**: weicht sie um mehr als **5 %** ab und steht der Messwert dabei **länger als fünf Minuten unverändert**, wird der Sollwert neu geschrieben und unter **Letzte Aktion** protokolliert. Hält die Abweichung an, wird der Wechselrichter nach weiteren fünf Minuten aus dem Regelmodus genommen — **Output 0 W, Timer-Toggle, Modus Disabled** — und im nächsten Zyklus von **Fall D** regulär zurückgeholt (Timer-Toggle + Modus `'1'`), woraufhin der PI wieder hochläuft. Das ist derselbe Weg, der einen aus dem Tritt geratenen Wechselrichter auch sonst wiederherstellt, und erzeugt eine echte Modus-Flanke statt den bestehenden Modus auf sich selbst zu schreiben. Zusätzlich erscheint eine Fehlermeldung im Status-Tab; das wiederholt sich im selben Abstand, bis der Ausgang dem Limit wieder folgt. Die Dauer stammt aus dem Zeitstempel der letzten Wertänderung des Ist-Sensors — er steht damit für „Wert seit dann unverändert" und macht eine eigene Uhr überflüssig; ein Ist-Sensor, der um den abweichenden Wert rauscht, löst entsprechend nicht aus. Die Prüfung gilt **ausschließlich** für diesen gesättigten PI-Zustand: Zone 0, AC Laden und Tarif-Laden halten den Ausgang bewusst unterhalb ihres jeweiligen Limits und sind ausgenommen, ebenso die Totbänder. Ohne konfigurierten oder verfügbaren Ist-Leistungssensor findet keine Prüfung statt. Gegenstück zur Nullungs-Bestätigung aus Punkt 10 — dort wird eine kommandierte 0 verifiziert, hier eine kommandierte Leistung.
15. **Ruhezustand.** Alle Übergänge, in denen die Regelung ruht — Fall B, C, F, TM, Lade-Ende (HT, H) und Fall I ohne aktiven Zyklus, und Stillstands-Recovery aus Punkt 14 — schreiben Output 0 W und den **Ruhemodus**. Standard ist Modus `'0'`; Modus `'0'` wird sonst nur von `Regelung aktiv = Aus` (Punkt 13) geschrieben. Mit dem Schalter **Ruhezustand in Modus 1** im Debug-Tab ist der Ruhemodus stattdessen `'1'` mit 0 W, Modus `'0'` wird dann nur noch beim Ausschalten der Regelung geschrieben. Die Ruhe in Modus `'1'` trägt ein eigenes Zustandsflag: in ihr läuft kein PI, die Instanz zählt nicht zum Entlade-Pool der Verteilung, Fall C, F und TM lösen nicht erneut aus, und Fall E, A, D, 0A, G und GT holen sie zurück wie aus Modus `'0'`. Der Betriebsmodus-Sensor zeigt `rest_discharge`. Gedacht für Geräte, die in Modus `'0'` trotz der [App-Einstellungen](#voraussetzungen) nicht laden. **Nicht empfohlen:** das Gerät verlässt die Fernsteuerung nur noch bei ausgeschalteter Regelung. Umschalten wirkt im nächsten Zyklus — eine ruhende Instanz wechselt dann einmal in den anderen Ruhemodus.
...
Alle Entitätsnamen kommen aus den Übersetzungen und erscheinen in der Sprache von Home Assistant (Deutsch oder Englisch). Die in der Tabelle genannten Entity-IDs gelten für eine deutschsprachige Neuinstallation; bestehende IDs ändern sich durch ein Update **nicht**, weil sie in der Entitätsregistrierung stehen.
...
| 9 | `safety_stop` | Sicherheitsstopp (SOC-Minimum) | Zone 3 — SOC auf oder unter der Zone-3-Schwelle, Modus `0`, Output 0 W |
| 10 | `pv_direct` | PV-Direktnutzung | sonst — Zone 2 bei Tag, Wechselrichter auf PV-Vorrang, Ausgangsleistung 0 |

### Sprache

Entitätsnamen und die Zustände der drei Enum-Sensoren (Betriebszustand, Aktiver Fall, Betriebsmodus) übersetzt Home Assistant selbst — sie folgen der Sprache des angemeldeten Benutzers. Zonen-Label, Modustext, letzte Aktion und Fehlermeldungen kann Home Assistant nicht übersetzen, weil sie Attribute bzw. Panel-Inhalte sind; sie werden von der Integration gerendert und folgen der **Sprache der Home-Assistant-Instanz** (Einstellungen → System → Allgemein). Weicht die persönliche Sprache eines Benutzers davon ab, erscheinen diese Texte in der Instanzsprache.

Für Automationen sind die sprachneutralen Schlüssel die richtige Quelle: der Zustandswert der Enum-Sensoren und das Attribut `action_key` der letzten Aktion.
