...
- **FAQ um vier Einträge und die Fehlerbehebung um einen erweitert, ausgelöst durch Discussion #31 und #30.** Neu im FAQ: (1) Betrieb ohne eigene PV-Panels am Gerät — Überschuss und die daran hängende Forecast-Erzwingung/Austritts-Sperre können nicht eintreten, die Nachtabschaltung ist bei `PV ≡ 0` dauerhaft aktiv und sperrt Zone 2, und von den vier Trigger-Quellen (Netz, Solar, SOC, Modus) bleibt praktisch nur der Netzsensor, weshalb der periodische Trigger in diesem Aufbau eingeschaltet gehören; (2) Vorzeichenbedeutung des Offsets — Zielwert am Netzzähler, negativ = Einspeise-Sicherheitsabstand; (3) warum AC Laden und die Entladezonen getrennte Offsets und P/I-Faktoren haben (zwei Regelstrecken, Ladenetzteil mit ~25-s-Flanke gegen Wechselrichter) und deshalb nicht gleichgesetzt werden sollten; (4) Abgrenzung „Max. Entladestrom Zone 1" (A, Batterieseite) gegen „Hard-Limit Z1" (W, AC-Ausgang, PI-Regelgröße). Neu in der Fehlerbehebung: Netz-Leistungssensor erscheint nicht in der Auswahl, weil der Selector auf `device_class: power` filtert und Template-Summensensoren diese Geräteklasse oft nicht tragen. Zwei bestehende Einträge daran angeglichen: die Frage zur Forecast-Erzwingung nennt jetzt die Voraussetzung eigener Panels am DC-Eingang (sie beschrieb ein Zone-0-Feature, dessen Nichtverfügbarkeit erst der neue AC-only-Eintrag erklärt), und die Frage zur nächtlichen Entladung benennt die Auslösebedingung `PV < PV-Ladereserve` statt einer bloßen Umschreibung.
- **Beschreibung des periodischen Triggers ergänzt** (`panel.de.json`, `panel.en.json`, README-Parametertabelle): nannte als Anwendungsfall bisher nur stabile Haushalte mit selten springendem Netzbezug. Der zweite Fall — Aufbauten ohne eigene PV am Gerät, in denen der Solar-Sensor als Trigger-Quelle komplett ausfällt — fehlte, obwohl er der gewichtigere ist; in Discussion #31 hat der Reporter den Trigger genau deswegen ausgelassen.

### Behoben
- **Regelung konnte im gesättigten Zustand dauerhaft verstummen** (Issue #32). Steht der Sollwert des Standard-PI am oberen Limit und besteht der Netzfehler in dieselbe Richtung fort (`at_max_limit ∧ grid_error > 0`), ist der Guard dauerhaft erfüllt — der Regler schreibt nie wieder. Bleibt der Wechselrichter in genau diesem Zustand stehen, ohne das Modus-Register zu ändern, erreicht ihn kein neuer Befehl mehr, und Fall D (Recovery) greift nicht, weil er einen Modus außerhalb von `'1'`/`'3'` voraussetzt. Feldbeleg unter v2.3.1: 19 h Ausgang 0 W bei aktivem Entladezyklus, SOC 93–100 % und durchgehendem Netzbezug von 100–400 W, `letzte Aktion` eingefroren auf `PI: 1199 → 1200 W`, Betriebsmodus durchgehend „INV Discharge PV Priority"; keine Selbstheilung, erst ein Hardware-Neustart des Wechselrichters beendete den Zustand — „Disabled" erschien erst danach, woraufhin Fall D sofort griff und der PI binnen ~25 s auf 1200 W hochlief. Da es keinen Schreibversuch gab, konnte auch nichts fehlschlagen und gemeldet werden. Neue Methode `_check_output_stall()` prüft ausschließlich in diesem gesättigten Zweig die tatsächliche Ausgangsleistung gegen das Limit: weicht sie um mehr als `OUTPUT_STALL_DEVIATION` (5 %) ab und steht der Messwert dabei länger als `OUTPUT_STALL_SECONDS` (300 s) unverändert, wird der Sollwert neu geschrieben; bleibt die Abweichung bestehen, wird der Wechselrichter im selben Abstand aus dem Regelmodus genommen (Integral-Reset, Output 0, Timer-Toggle, Modus `'0'`) und im Folgezyklus von Fall D regulär zurückgeholt, dessen Bedingung damit erfüllt ist — eine echte Modus-Flanke `'0' → '1'` statt einer Wiederholung des bereits stehenden `'1'`, also derselbe Weg, der den Feldfall nach dem Hardware-Neustart beendet hat. Dazu eine Fehlermeldung über `_output_warning`. Die Dauer kommt aus `state.last_updated` des Ist-Sensors: der Zeitstempel rückt nur bei einer Wertänderung vor und steht damit exakt für „Wert seit dann unverändert" — ein hängender Ausgang ist so ohne eigene Uhr messbar, ein um den abweichenden Wert rauschender Sensor löst nicht aus. Die übrigen drei PI-Pfade sind bewusst ausgenommen: Zone 0 klemmt den Entladestrom auf 2 A, Tarif-Laden fährt eine feste Ladeleistung und die Totbänder halten den Sollwert mitten im Bereich — dort ist eine Abweichung vom jeweiligen Limit der Normalfall. Ohne konfigurierten oder verfügbaren Ist-Leistungssensor findet keine Prüfung statt (`coordinator.py`, `const.py`)
