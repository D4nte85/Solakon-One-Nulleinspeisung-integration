...
### Behoben
- Fall-H-Austritt (AC Laden Ende): Vergleich `eigener Output ≤ 0 W` (aus V309, gehärtet in v2.2.7 gegen isolierte Ein-Sekunden-Nullwerte durch Modbus-Rauschen) auf `abs(actual) <= S_SELF_ADJUST_TOL` (Standard 2 W, dieselbe Toleranz wie die PI-Konvergenzprüfung) geändert. Der einseitige `≤ 0`-Vergleich machte den Guard empfindlicher als vorher beabsichtigt: jeder noch so kleine negative Ausreißer (z. B. −0,1 W) erfüllt die Bedingung jetzt ebenfalls, während der vorherige exakte `== 0`-Vergleich solche Ausreißer nie getroffen hätte. Bei Setups mit häufigem AC-Laden (insbesondere reine AC-Anbindung ohne PV, die proportional mehr Zeit in Fall G/H verbringt als ein PV-geführtes Setup) beobachtet als periodischer Moduswechsel 3↔1 alle ~15–20 s während aktivem AC-Laden, mit hartem Output-Reset auf 0 W bei jedem Fehlauslöser (Feldbeleg 27.08., Kumpel-Installation, Fehler aufgetreten unter v2.2.8). Fix: symmetrisches Toleranzband um 0 W statt einseitigem Vergleich — fängt Rauschen in beide Richtungen ab, ohne bei jedem winzigen negativen Ausreißer während echtem Weiterladen auszulösen (`coordinator.py`)
